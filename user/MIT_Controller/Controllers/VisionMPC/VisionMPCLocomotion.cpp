...
void VisionMPCLocomotion::run(ControlFSMData<float>& data, 
  ...
  (void)idx_map;
  time += data.controlParameters->controller_dt + 1e-6;
  ...
  for(int foot = 0; foot < 4; foot++)
  {
    ...
    if(swingState > 0) // foot is in swing
    {
      ...
      using namespace std;
      cout << "Foot Position: " << pFoot_des[0] << pFoot_des[1] << pFoot_des[2] << pFoot_des[3] << endl;
      cout << "Foot Velocity: " << vFoot_des[0] << vFoot_des[1] << vFoot_des[2] << vFoot_des[3] << endl;
      cout << "Foot Acceleration: " << aFoot_des[0] << aFoot_des[1] << aFoot_des[2] << aFoot_des[3] << endl;
  ...
  contact_state = gait->getContactState();
  using namespace std;
  cout << "Body Position: " << pBody_des[0] << pBody_des[1] << pBody_des[2] << endl;
  cout << "Body Velocity: " << vBody_des[0] << vBody_des[1] << vBody_des[2] << endl;
  cout << "Body RPY: " << pBody_RPY_des[0] << pBody_RPY_des[1] << pBody_RPY_des[2] << endl;
  cout << "Body Ori Vel: " <<   vBody_Ori_des[0] << vBody_Ori_des[1] << vBody_Ori_des[2] << endl;
  // END of WBC Update
}
...
void VisionMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
  ...
  for(int leg = 0; leg < 4; leg++)
  {
    ...
    // Update for WBC
    Fr_des[leg] = f;
    std::cout << "Foot Forces: " << Fr_des[0] << Fr_des[1] << Fr_des[2] << Fr_des[3] << std::endl;
